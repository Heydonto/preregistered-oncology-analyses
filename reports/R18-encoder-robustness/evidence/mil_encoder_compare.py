...
EMB_DIM = 1024          # Phikon-v2 and dinov2-large are both 1024-d; asserted at load
TILE_CAP = 4096
LR = 2e-4
WD = 1e-5
EPOCHS = 20
...
def run(feat_dir: str, folds_mode: str, mode: str) -> dict:
    ...
    for sid in need:
        ...
        if e.shape[1] != EMB_DIM:
            raise RuntimeError(f"{sid}: embedding dim {e.shape[1]} != {EMB_DIM}")
    ...
    print(f"[{feat_dir}/{folds_mode}/{mode}] cached {len(cache)}/{len(need)} slides, "
          f"{sum(v.nbytes for v in cache.values())/2**30:.1f} GiB, encoders={sorted(enc_seen)}",
          flush=True)
    if len(enc_seen) > 1:
    ...
    def train(items, seed):
        ...
        model = build_model(n_out).to(device).train()
    ...
    res = {"feat_dir": feat_dir, "folds": folds_mode, "mode": mode,
           "encoder": sorted(enc_seen), "n_patients": len(pats),
